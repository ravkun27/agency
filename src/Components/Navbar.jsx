...
const Navbar = () => {
  ...
  return (
    ...
    <nav className='bg-white md:px-14 p-4 max-w-screen-2xl border-b mx-auto text-primary fixed top-0 right-0 left-0'>
        <div className='text-xl container mx-auto flex justify-between items-center font-medium'>
            <div className='flex space-x-14 items-center'>
                <a href="/" className='text-2xl font-semibold flex items-center space-x-3 text-primary'><img src={logo} alt="" className='w-10 pb-1 inline-block items-center'/><span>SP</span></a>

                {/* showing navitems using map */}
                <ul className='hidden md:flex space-x-12'>
                    {
                        navItems.map(({link,path}) => <Link spy={true} smooth={true} activeClass='active' offset={-100} key={link} to={path} className='block hover:text-gray-300 cursor-pointer'>{link}</Link>)
                    }
                </ul>
            </div>
    ...
    {
                        navItems.map(({link,path}) => <Link spy={true} smooth={true} activeClass='active' offset={-80} key={link} to={path} className='block hover:text-gray-300 text-white cursor-pointer' onClick={toggleMenu}>{link}</Link>)
                    }
    </div>
    </>
  )
